...
class Cluster:
    ...
    def inventory(self):
        req = requests.get(
            ...
            + r'wasm/contracts/{0}/store?query_msg=%7B"cluster_state":%7B%7D%7D'.format(self.cluster_contract)
        ).json()
        ...
        return df[["asset_types", "address", "prices", "inventory", "target_amount"]]

    # returns df of cluster target msg
    def target(self):
        req = requests.get(
            ContractInfo().clusters[self.net]["chain_url"]
            + r'wasm/contracts/{0}/store?query_msg=%7B"target":%7B%7D%7D'.format(self.cluster_contract)
        ).json()
        asset_types = [list(i["info"].keys())[0] for i in req["result"]["target"]]
        df = pd.DataFrame.from_dict(req["result"]["target"])
        df["asset_types"] = asset_types
        ...
        return df.rename(columns={"amount": "target_amount"}).drop("info", axis=1)[
            ["asset_types", "address", "target_amount"]
        ]
    ...
    def config(self):
        req = requests.get(
            ContractInfo().clusters[self.net]["chain_url"]
            + r'wasm/contracts/{0}/store?query_msg=%7B"config":%7B%7D%7D'.format(self.cluster_contract)
        ).json()
